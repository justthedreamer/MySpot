...
public sealed class ChangeReservationLicensePlateHandler(IWeeklyParkingSpotRepository repository)
    : ICommandHandler<ChangeReservationLicensePlate>
{
    public async Task HandleAsync(ChangeReservationLicensePlate command)
    {
        ...
        if (weeklyParkingSpot == null) throw new ReservationNotFoundException(command.ReservationId);
