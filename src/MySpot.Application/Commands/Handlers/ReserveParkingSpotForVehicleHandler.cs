    ...
    : ICommandHandler<ReserveParkingSpotForVehicle>
{
    public async Task HandleAsync(ReserveParkingSpotForVehicle command)
    {
        ...
        if (parkingSpotToReserve is null) throw new WeeklyParkingSpotNotFoundException(command.ParkingSpotId);

        var reservation = new VehicleReservation(command.ReservationId, command.ParkingSpotId, command.EmployeeName,
            command.LicensePlate, command.Date, command.Capacity);

        parkingReservationService.ReserveSpotForVehicle(weeklyParkingSpots, JobTitle.Employee, parkingSpotToReserve,
            reservation);
